...
const HistorialM = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:8082/showHistoriales')
        .then((response) => {
          // Almacenar los datos en el estado del componente
          const data = response.data;
          getData(data)
          const number = data[0].n_serie;
    ...
    const getHistorial = (number) => {
        axios.post('http://localhost:8082/showHistorialM',{
            n_serie: number
        })
        .then((response) => {
          // Almacenar los datos en el estado del componente
    ...
    return (
        <div>
            <Navbar />
            <h1 className="text-center py-12 text-3xl italic">Historial de incidencias de motores</h1>
            <div className="flex">
                <div className="form flex flex-col gap-8 px-40 w-1/3">
                <select onChange={handleChange}>
                    {data && data.map(elemento => (
                        <option key={elemento.n_serie}>{elemento.n_serie}</option>
                    ))}
                </select>
                </div>
